...
/* -----------------работает не быстро..((---------------------------
int effective_method(double* a,double* inv, int n){
	double s,sum;
	double norm_a, norm_x, norm;
	double sp,sp2;
	double tmp;
    ...
    norm_m = max;
    if (fabs(norm_m)<1e-16) {std::cout<<"Bad matrix"<<std::endl; return -1;}


    //double eps=(1e-15) * norm(n, a,inv);
	for(int i=0;i<n;++i){
		if (i==n-1){
            //if (a[i*n+i]<1e-15) {
            //    std::cout<<"Bad matrix!"<<std::endl; return -1; 
            //}
			continue;
		}
		...
		norm_a=sqrt(s+(a[i*n+i]*a[i*n+i]));
		//if (norm_a<1e-256) {std::cout<<"Bad matrix"<<std::endl; return -1; }
		a[i*n+i]-=norm_a;
		norm_x=sqrt(a[i*n+i]*a[i*n+i]+s);
		if (norm_x<1e-256){a[i*n+i]=norm_a; continue; }        
		...
		for (k=0;k<n;++k){
			...
			for (int j=i;j<n;++j){
				if (k!=i) sp+=a[j*n+i]*a[j*n+k];
				sp2+=a[j*n+i]*inv[j*n+k];			
			}
			
			for (int j=i;j<n;++j){
				inv[j*n+k]-=(2.0*sp2*a[j*n+i]);
				if (k!=i) a[j*n+k]-=(2.0*sp*a[j*n+i]);
				//if (k==n-1 && j!=i) a[j*n+i]=0.0;
		...
		a[i*n+i]=norm_a;
		//if (i!=(n-1)) a[i*n+i+1]=0.0;
        //if (a[i*n+i]<1e-15) {std::cout<<"Bad matrix"<<std::endl; return -1;}
